...
export function processMishnahEnglishText(text: string): string {
  ...
  processed = processed
    ...
    .replace(/R' Jacob\b/g, "R' Ya'akov")
    .replace(/R' Jonah\b/g, "R' Yonah")
    .replace(/R' Jeremiah\b/g, "R' Yirmeyah")
    .replace(/R' Jonathan\b/g, "R' Yonatan")
    .replace(/R' Isaac\b/g, "R' Yitzḥak")
    .replace(/R' Samuel\b/g, "R' Shmuel")
    .replace(/R' Nathan\b/g, "R' Natan")
    .replace(/R' Phineas\b/g, "R' Pinḥas")
    // Non-biblical names: normalize globally
    .replace(/\bSimeon\b/g, 'Shimon')
